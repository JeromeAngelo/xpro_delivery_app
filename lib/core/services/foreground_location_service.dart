...
static Future<bool> startTracking({
  ...
  try {
    ...
    try {
      ...
      final bool serviceRunning = isRunning == true || isRunning.toString() == 'true';
      if (serviceRunning) {
    ...
    await prefs.setString('tracking_start_time', DateTime.now().toIso8601String());
    ...
    final bool startedSuccessfully = isRunningNow == true || isRunningNow.toString() == 'true';
...
static Future<bool> stopTracking() async {
  try {
    ...
    final dynamic isRunning = await FlutterForegroundTask.isRunningService;
    final bool isStillRunning = isRunning == true || isRunning.toString() == 'true';
...
class LocationTaskHandler extends TaskHandler {
  ...
  Future<void> onStart(DateTime timestamp, TaskStarter starter) async {
    debugPrint('🚀 LocationTaskHandler started at ${timestamp.toIso8601String()}');
  }
  ...
  void onRepeatEvent(DateTime timestamp) {
    ...
    debugPrint('🔄 Background location update #$_updateCount at ${now.toIso8601String()}');
  ...
  Future<void> _performLocationUpdate(DateTime now) async {
    try {
      ...
      final pbUrl = prefs.getString('tracking_pb_url') ?? 
      ...
      if (permission == LocationPermission.denied || 
        ...
        _updateNotification(
          'Permission Required',
          'Location permission needed for tracking',
        );
        return;
      }
      ...
      try {
        ...
        debugPrint('⚠️ Failed to save coordinate: $e');
      }
      
      // Update notification with success status
      if (updated) {
        final timeSince = _lastUpdate != null 
            ? now.difference(_lastUpdate!).inMinutes 
            : 0;
        _updateNotification(
  ...
  Future<void> onDestroy(DateTime timestamp) async {
    debugPrint('🛑 LocationTaskHandler destroyed at ${timestamp.toIso8601String()}');
  }

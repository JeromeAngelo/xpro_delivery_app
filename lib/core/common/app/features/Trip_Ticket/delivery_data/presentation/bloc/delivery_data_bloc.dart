...
class DeliveryDataBloc extends Bloc<DeliveryDataEvent, DeliveryDataState>
  ...
  Future<void> _onGetDeliveryDataByTripId(
    ...
    debugPrint(
      '🔍 OFFLINE-FIRST: Getting delivery data for trip ID: ${event.tripId}',
    );
    emit(const DeliveryDataLoading());

    await executeOfflineFirst(
      localOperation: () async {
        final result = await _getDeliveryDataByTripId.loadFromLocal(
          event.tripId,
        );
      ...
      onError:
          (error) => emit(DeliveryDataError(message: error, statusCode: '0')),

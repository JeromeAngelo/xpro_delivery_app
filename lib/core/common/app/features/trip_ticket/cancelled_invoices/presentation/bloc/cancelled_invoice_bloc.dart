...
Future<void> _onLoadCancelledInvoicesByTripId(
  ...
  debugPrint('🔍 Loading cancelled invoices for trip: ${event.tripId}');
  emit(const CancelledInvoiceLoading());

  try {
    // Load cancelled invoices directly from repository
    final result = await _loadCancelledInvoicesByTripId(event.tripId);

    result.fold(
      (failure) {
        debugPrint('❌ Failed to load cancelled invoices: ${failure.message}');
        emit(CancelledInvoiceError(failure.message));
      },
      (cancelledInvoices) {
        if (cancelledInvoices.isEmpty) {
          debugPrint('📭 No cancelled invoices found for trip: ${event.tripId}');
          emit(CancelledInvoicesEmpty(event.tripId));
        } else {
          debugPrint('✅ Loaded ${cancelledInvoices.length} cancelled invoices');
          emit(CancelledInvoicesLoaded(cancelledInvoices));
        }
      },
    );
  } catch (e, st) {
    debugPrint('❌ Exception while loading cancelled invoices: $e\n$st');
    emit(CancelledInvoiceError(e.toString()));
  }
}

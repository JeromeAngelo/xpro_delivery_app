...
import 'package:x_pro_delivery_app/core/common/app/features/trip_ticket/delivery_collection/domain/usecases/delete_collection.dart';
import 'package:x_pro_delivery_app/core/common/app/features/trip_ticket/delivery_collection/domain/usecases/get_collection_by_id.dart';
...
class CollectionsBloc extends Bloc<CollectionsEvent, CollectionsState> with OfflineFirstMixin<CollectionsEvent, CollectionsState> {
 ...
 Future<void> _onGetCollectionsByTripId(
  ...
  debugPrint('🌐 ONLINE: Fetching collections for trip: ${event.tripId}');
  emit(const CollectionsLoading());

  try {
    // Fetch collections from remote only
    final result = await _getCollectionsByTripId(event.tripId);

    result.fold(
      (failure) {
        debugPrint('❌ Failed to fetch collections: ${failure.message}');
      ...
      (collections) {
        if (collections.isEmpty) {
          debugPrint('⚠️ No collections found for trip: ${event.tripId}');
          emit(CollectionsEmpty(event.tripId));
        } else {
          debugPrint('✅ Collections loaded from remote for trip: ${event.tripId}');
          final newState = CollectionsLoaded(
            collections: collections,
            isFromCache: false, // purely online
          );
          emit(newState);
          _cachedState = newState; // optional: cache for UI continuity
        }
      },
    );
  } catch (e, st) {
    debugPrint('❌ Unexpected error fetching collections: $e\n$st');
    emit(CollectionsError(
      message: e.toString(),
      errorCode: '500',
    ));

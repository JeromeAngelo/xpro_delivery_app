...
class CollectionModel extends CollectionEntity {
  ...
  // Convenience relation IDs (query-friendly)
  @Property()
  ...
  final deliveryReceipt = ToOne<DeliveryReceiptModel>();

  // --------------------------------------------------------------------------
  // CONSTRUCTOR (TripModel-style)
  // --------------------------------------------------------------------------
  ...
  CollectionModel({
    ...
    deliveryDataId = deliveryDataModel?.id;
    tripId = tripData?.id;
    ...
    if (invoiceData != null) invoices.add(invoiceData);
    if (invoicesList != null) invoices.addAll(invoicesList);
    if (deliveryReceiptData != null) {
      deliveryReceipt.target = deliveryReceiptData;
    }
  }
  ...
  factory CollectionModel.fromJson(DataMap json) {
    ...
    debugPrint(
      '   - Customer: ${customerModel?.id ?? "null"} (${customerModel?.name ?? "null"})',
    );
    debugPrint(
      '   - Invoice: ${invoiceModel?.id ?? "null"} (Amount: ${invoiceModel?.totalAmount ?? "null"})',
    );

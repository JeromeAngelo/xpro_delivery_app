...
class _UpdateTimelineViewState extends State<UpdateTimelineView>
  ...
  void _initializeBlocs() {
   ...
   void _loadInitialData() {
    ...
    debugPrint('📊 TIMELINE: Current cached states - Delivery: ${_cachedDeliveryState?.runtimeType ?? 'null'}, Trip: ${_cachedTripState?.runtimeType ?? 'null'}');
  ...
  void _setupDataListeners() {
    _authSubscription = _authBloc.stream.listen((state) {
      ...
      if (state is UserTripLoaded || state is UserByIdLoaded || state is UserDataRefreshed) {
      ...
      if (state is AuthLoading || state is AuthError) {
        debugPrint('⚠️ Ignoring ${state.runtimeType} - keeping cached auth data visible');
      }
    });
    ...
    _deliveryDataSubscription = _deliveryDataBloc.stream.listen((state) {
      ...
      if (state is DeliveryDataByTripLoaded || state is AllDeliveryDataLoaded) {
        if (state is DeliveryDataByTripLoaded) {
          ...
          debugPrint('✅ Delivery data cached: ${state.deliveryData.length} items');
        }
    ...
    _tripUpdateSubscription = _tripUpdatesBloc.stream.listen((state) {
      ...
      if (state is TripUpdatesError) {
        debugPrint('⚠️ Trip updates network error, using cached data: ${state.message}');
        // Only try fallback if we have no cached data
      ...
      if (state is TripUpdatesLoading) {
        debugPrint('⚠️ Ignoring loading state - keeping cached trip updates visible');
      }
    });
  }
      ...
      listeners: [
        BlocListener<DeliveryDataBloc, DeliveryDataState>(
          listener: (context, state) {
            ...
            if (state is DeliveryDataByTripLoaded) {
              setState(() => _cachedDeliveryState = state);
              debugPrint('📦 Delivery data listener: ${state.deliveryData.length} items cached');
        ...
        BlocListener<TripUpdatesBloc, TripUpdatesState>(
          listener: (context, state) {
            ...
            if (state is TripUpdatesLoaded) {
              setState(() => _cachedTripState = state);
              debugPrint('📝 Trip updates listener: ${state.updates.length} items cached');
            } else if (state is TripUpdatesError) {
              debugPrint('⚠️ Trip updates network error, keeping cached data: ${state.message}');
              // Only fallback if we have no cached data at all
              if (_cachedTripState == null) {
                final tripId = widget.tripId;
                debugPrint('🔄 No cached data, falling back to local trip updates');
                _tripUpdatesBloc.add(LoadLocalTripUpdatesEvent(tripId));
              }
            }
          },
  ...
  return BlocBuilder<DeliveryDataBloc, DeliveryDataState>(
    ...
    builder: (context, deliveryDataState) {
      ...
      if (_cachedDeliveryState != null) {
        ...
        debugPrint('📱 Timeline using cached delivery state: ${_cachedDeliveryState.runtimeType}');
      ...
      // Only use current state if we have no cache
        ...
        debugPrint('📱 Timeline using current delivery state: ${deliveryDataState.runtimeType}');
      }

      // Only show loading if we have no cached data at all
      if (effectiveDeliveryState is DeliveryDataLoading && _cachedDeliveryState == null) {
        return const Center(
          child: Column(
      ...
      if (effectiveDeliveryState is DeliveryDataError && _cachedDeliveryState == null) {
        return Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
                ...
                child: BlocBuilder<TripUpdatesBloc, TripUpdatesState>(
                  ...
                  builder: (context, tripUpdatesState) {
                    ...
                    if (_cachedTripState != null) {
                      ...
                      debugPrint('📱 Timeline using cached trip state: ${_cachedTripState.runtimeType}');
                    }
                    // Only use current state if we have no cache
                    else {
                      effectiveTripState = tripUpdatesState;
                      debugPrint('📱 Timeline using current trip state: ${tripUpdatesState.runtimeType}');
                    }
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        showOfflineIndicator ? 'No Arrived Deliveries (Offline)' : 'No Arrived Deliveries',

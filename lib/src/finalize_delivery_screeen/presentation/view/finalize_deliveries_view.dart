    ...
    if (tripId == null) {
      try {
        ...
        if (storedData != null) {
          ...
          debugPrint('📋 CHECKLIST: User data from SharedPreferences: $userData');
          
          // Check for trip data in nested structure
          final tripData = userData['trip'] as Map<String, dynamic>?;
          if (tripData != null && tripData['id'] != null) {
            tripId = tripData['id'];
            debugPrint('🎫 CHECKLIST: Found trip ID in nested structure: $tripId');
          } else {
    ...
    if (!_isInitialized) {
      _authSubscription = _authBloc.stream.listen((state) {
        ...
        if (state is UserTripLoaded && state.trip.id != null) {
          debugPrint('🎫 CHECKLIST: Loading checklist for trip from auth: ${state.trip.id}');
      ...
      _checklistSubscription = _endTripChecklistBloc.stream.listen((state) {
        ...
        if (state is EndTripChecklistLoaded) {
          ...
          debugPrint('✅ CHECKLIST: Loaded with ${state.checklists.length} items');
          
          // Log each checklist item for debugging
          for (int i = 0; i < state.checklists.length; i++) {
            final item = state.checklists[i];
            debugPrint('   📝 Item ${i + 1}: ${item.id} (Checked: ${item.isChecked})');
          }
        ...
        return SliverList(
          delegate: SliverChildBuilderDelegate(
            (context, index) {
              ...
              debugPrint('📋 CHECKLIST: Building item ${index + 1}: ${item.id}');
              return Padding(
                ...
                child: EndTripChecklistTile(
                  ...
                  onChanged: (value) {
                    debugPrint('✅ CHECKLIST: Item ${item.id} toggled to $value');
